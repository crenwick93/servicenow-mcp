...
def main(argv: List[str]) -> int:
    if len(argv) < 2:
        print("Usage: python3 search_problems.py <keyword1> [keyword2 ...]", file=sys.stderr)
        return 2
    ...
    keywords = argv[1:]
    result = search_problems(
        server_config,
        auth,
        SearchProblemsParams(keywords=keywords, limit=20, offset=0),
    )

    # Pretty-print JSON output for easy reading
